...
class HomeController extends Controller
{
    ...
    public function vendorIndex(Request $request)
     ...
     if(Session::has('vendor_grocery'))
     {
        ...
        $current2 = date('d-m-Y', strtotime($current.' + '.$day.' days'));
    	 ...
    	 $total_earnings = DB::table('orders')
    	   ...
    	   $total_cash = DB::table('orders')
    	                    ...
    	                    ->where('order_status','=','Completed')
    	                    ->sum('rem_price');
    	   $total_amount = DB::table('comission')
    	                    ->where('vendor_id',$vendor->vendor_id)
        ...
        return view('vendor.index', compact("vendor_email", "vendor", "total_earnings", "total_users", "ongoing","complete","cityadmin","orders","user","comment","cancel","total_cash","currency","app_share","daily_count","today","recent_order","reffer_arning","created_at","total_amount"));
