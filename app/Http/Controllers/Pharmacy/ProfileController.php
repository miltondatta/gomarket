...
class ProfileController extends Controller
{
    public function pharmacyProfile(Request $request)
    {
     ...
     if(Session::has('vendor_pharma'))
     {
    	 ...
    	 $profile= DB::table('vendor')
           ...
           return view('pharmacy.profile',compact("vendor_email","profile"));
    }
	else
	 {
    ...
    public function pharmacyEditvendor(Request $request)
    {
         ...
         return view('pharmacy.editvendor.editvendor',compact("vendor_email","vendor_id","vendor"));
    }

    public function pharmacyvendorUpdateProfile(Request $request)
    {
     ...
     if(Session::has('vendor_pharma'))
     {
        ...
        if($vendorChangeProfile){
             ...
             session::put('vendor_pharma',$vendor_email);
    ...
    public function pharmacyvendorChangePass(Request $request)
    {
       if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
     if(Session::has('vendor_pharma'))
     {
        ...
        $vendor_email=Session::get('vendor_pharma');
         $pass= DB::table('vendor')
           ...
           return view('pharmacy.change_pass',compact("vendor_email","pass"));
    }
	else
	 {
	    return redirect()->route('vendorlogin')->withErrors('please login first');
   ...
   public function pharmacyvendorChangePassword(Request $request)
    {
       if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
      if(Session::has('vendor_pharma'))
     {
     ...
     public function pharmacyvendorLogout(Request $request)
     {
      if(Session::has('vendor_pharma'))
     {
          $request->session()->flush();
           return redirect()->route('pharmacylogin');
	 }
	else
	 {
	    return redirect()->route('vendorlogin')->withErrors('please login first');
