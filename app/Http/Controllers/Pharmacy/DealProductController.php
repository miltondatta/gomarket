...
{
    public function pharmacydealroduct(Request $request)
    {
    	...
    	return view('pharmacy.deal_product.deal_list', compact("vendor_email", "vendor","deal_p","currentdate"));
    }

    
     public function pharmacyAddDealproduct(Request $request)
    {
         $vendor_email=Session::get('vendor_pharma');
        ...
        return view('pharmacy.deal_product.add_deal',compact("deal", "vendor_email","vendor"));
     }
    
     public function pharmacyAddNewDealproduct(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
         if(Session::has('vendor_pharma'))
        {
             $vendor_email=Session::get('vendor_pharma');
             $vendor=DB::table('vendor')
                    ->where('vendor_email',$vendor_email)
    ...
    public function pharmacyEditDealproduct(Request $request)
    {
         $deal_id = $request->id;
         $deal = DB::table('resturant_variant')
                ->join('resturant_product','resturant_variant.product_id','=','resturant_product.product_id')
        ...
        return view('pharmacy.deal_product.edit_deal',compact("deal_p","vendor_email","deal","vendor"));
    }

    public function pharmacyUpdateDealproduct(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
        $deal_id = $request->id;
       $variant_id = $request->variant_id;
    ...
    public function pharmacyDeleteDealproduct(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
        $deal_id = $request->id;
