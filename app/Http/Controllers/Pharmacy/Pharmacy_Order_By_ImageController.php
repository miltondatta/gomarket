...
class Pharmacy_Order_By_ImageController extends Controller
{
    use SendMail;
    use SendSms;
      public function parmacy_order_list(Request $request)
    {
       $vendor_email=Session::get('vendor_pharma');
             ...
             $list = DB::table('order_by_photo')
              ...
              ->get();
         return view('pharmacy.order_by_photo.user_orderlist', compact("vendor", "vendor_email","list"));   
    ...
    public function pharmacy_sel_product(Request $request)
    {
        $vendor_email=Session::get('vendor_pharma');
        $vendor=DB::table('vendor')
                    ->where('vendor_email',$vendor_email)
        ...
        $selected =  DB::table('list_cart')
                ->join('resturant_variant', 'list_cart.l_vid', '=', 'resturant_variant.variant_id')
        ...
        foreach($check as $ch){
          ...
          $products = DB::table('resturant_variant')
                ->join('resturant_product','resturant_variant.product_id', '=', 'resturant_product.product_id')
                ->whereNotIn('variant_id', $ch3)
               
                ->get();    
        
    	return view('pharmacy.order_by_photo.orderbyphoto', compact('vendor_email',"products","selected","u"));
        }else{
            $products = DB::table('resturant_variant')
                 ->join('resturant_product','resturant_variant.product_id', '=', 'resturant_product.product_id')
                ->get(); 
                
            return view('pharmacy.order_by_photo.orderbyphoto', compact('vendor_email',"products","selected","user_id","u"));    
        }
    ...
    public function pharmacy_added_product(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
        $vendor_email=Session::get('vendor_pharma');
         $uid = $request->user_id;
    ...
    for($i=0;$i<=($countprod-1);$i++)
     ...
     public function pharmacy_delete_product(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
        $id =$request->id;
     ...
     public function pharmacy_add_qty(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
        $id =$request->id;
        // var_dump($id);
        $qty = $request->stock;
         ...
         public function pharmacy_rejectorder(Request $request)
        {
            if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
         $ord_id=$request->id;
         $cause = $request->cause;
    ...
    public function parmacy_checkout(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));   
        $current = Carbon::now();
        $vendor_email=Session::get('vendor_pharma');
