...
{
    public function pharmacysubcat(Request $request)
    {
        ...
        ->paginate(10);
        return view('pharmacy.subcat.subcat',compact("vendor_email","subcat","vendor"));
    }
    
     public function pharmacyaddsubcat(Request $request)
    {
        ...
        $category= DB::table('resturant_category')
                ->where('vendor_id',$vendor->vendor_id)
                ->get();
         return view('pharmacy.subcat.addsubcat',compact("vendor_email","category","vendor"));
    }
    
    
        public function pharmacyAddNewsubcat(Request $request)
        {
    ...
    public function pharmacyEditsubcat(Request $request)
    {
       ...
       $subcat_id=$request->subcat_id;
    	 ...
    	 $category=DB::table('tbl_category')
    	        ->where('vendor_id',$vendor->vendor_id)
                ->get();		  
    	 return view('pharmacy.subcat.Editsubcat',compact("vendor_email","vendor","category","subcat_id","subcat"));


    }
    public function pharmacyUpdatesubcat(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
        $subcat_id=$request->subcat_id;
  ...
  public function pharmacydeletesubcat(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
        $subcat_id=$request->subcat_id;
