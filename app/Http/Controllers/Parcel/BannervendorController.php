...
class BannervendorController extends Controller
{
    public function resturantbannervendor(Request $request)
     ...
     {   
        ...
        ->leftjoin('resturant_category' , 'banner.bannerloc_id', '=', 'resturant_category.vendor_id')
        ->where('banner.vendor_id',$vendor->vendor_id)
        ->get();
        return view('parcel.banner.bannervendor',compact("vendor_email","banner","vendor"));
    ...
    public function resturantAddbannervendor(Request $request)
     ...
     if(Session::has('vendor_parcel'))
     {
        ...
        $category= DB::table('resturant_category')
                ...
                ->get();
         return view('parcel.banner.addbanner',compact("vendor_email","category","vendor"));
         }
    else
         {
            return redirect()->route('vendorlogin')->withErrors('please login first');
    ...
    public function resturantAddNewbannervendor(Request $request)
      ...
      if(Session::has('vendor_parcel'))
     {
        ...
        $insert = DB::table('banner')
                  ->insert(['vendor_id'=>$vendor_id,'keyword'=>$keyword,'bannerloc_id'=>$category_name,'banner_name'=>$banner_name,'banner_image'=>$banner_image,'created_at'=>$created_at]);
      ...
      {	
       ...
       $banner_id=$request->id;
    	 ...
    	 $category=DB::table('resturant_category')
    	        ->where('vendor_id',$vendor->vendor_id)
                ->get();		  
    	 return view('parcel.banner.Editbanner',compact("vendor_email","vendor","category","banner_id","banner"));
         }
    else
         {
            return redirect()->route('vendorlogin')->withErrors('please login first');
