...
{
    public function parcelarea(Request $request)
    {
    	...
    	if(Session::has('vendor_parcel'))
          {
    	         ...
    	         $area= DB::table('vendor_area')
    	                  ->join('area','vendor_area.area_id','=','area.area_id')
    	 		          ...
    	 		          ->get();
    	         return view('parcel.area.area',compact("vendor_email","area","vendor"));
          }
        else
             {
    ...
    public function parcelAddarea(Request $request)
    {
    ...
    if(Session::has('vendor_parcel'))
     {
    	...
    	return view('parcel.area.Addarea',compact("vendor_email","vendor","area"));
         }
    else
         {
            return redirect()->route('vendorlogin')->withErrors('please login first');
    ...
    public function parcelAddInsertarea(Request $request)
    {
    ...
    public function parcelEditarea(Request $request)
    {
    ...
    if(Session::has('vendor_parcel'))
     ...
     $area1 = DB::table('area')
                ...
                ->get();
	 return view('parcel.area.Editarea',compact("vendor_email","area","area_id","vendor","area1"));
      }
    else
      {
        return redirect()->route('vendorlogin')->withErrors('please login first');
    ...
    public function parcelUpdatearea(Request $request)
    {
      if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
    if(Session::has('vendor_parcel'))
     {
    ...
    public function parcelDeletearea(Request $request)
    {
      if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
     if(Session::has('vendor_parcel'))
     {   
