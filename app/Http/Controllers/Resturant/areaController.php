...
{
    public function restaurantarea(Request $request)
    {
    	...
    	if(Session::has('vendor_restaurant'))
          {
    	         ...
    	         $area= DB::table('vendor_area')
    	                  ->join('area','vendor_area.area_id','=','area.area_id')
    	 		          ...
    	 		          ->get();
    	         return view('resturant.area.area',compact("vendor_email","area","vendor"));
          }
        else
             {
    ...
    public function restaurantAddarea(Request $request)
    {
    ...
    if(Session::has('vendor_restaurant'))
     {
    	...
    	return view('resturant.area.Addarea',compact("vendor_email","vendor","area"));
         }
    else
         {
            return redirect()->route('vendorlogin')->withErrors('please login first');
    ...
    public function restaurantAddInsertarea(Request $request)
    {
    ...
    public function restaurantEditarea(Request $request)
    {
    ...
    if(Session::has('vendor_restaurant'))
     ...
     $area1 = DB::table('area')
                ...
                ->get();
	 return view('resturant.area.Editarea',compact("vendor_email","area","area_id","vendor","area1"));
      }
    else
      {
        return redirect()->route('vendorlogin')->withErrors('please login first');
    ...
    public function restaurantUpdatearea(Request $request)
    {
      if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
    if(Session::has('vendor_restaurant'))
     {
    ...
    public function restaurantDeletearea(Request $request)
    {
        if(Setting::valActDeMode()) return redirect()->back()->withErrors(trans('messages.Active_Demo_Mode'));
     if(Session::has('vendor_restaurant'))
     {   
