...
<html lang="en">
   ...
   <body>
      ...
      @include("web.restaurantcategory_slider")
               ...
               <table class="table table-striped shadow-lg saved_mob2" style="border-radius: 10px;">
                  <thead>
                     <tr>
                        ...
                        <th scope="col" style="width: 15%;">{{ __('messages.Qty')}}</th>
                  ...
                  <tbody >
                     ...
                     <tr>
                        <th scope="row ycart_th">
                           <br>
                           <img src="{{ url(''.$productdetails->product_image)}}" class="ycart_img">
                        ...
                        <td><i class="fas fa-trash ycart_trash" onclick="restaurantemovecartitem({{ $cartitem->cart_id}})"></i></td>
                     </tr>
                     <?php }else{
                     ...
                     ?> 
                        ...
                        <th scope="row ycart_th">
                           <br>
                           <img src="{{ url(''.$productdetails->product_image)}}" class="ycart_img" style="width:100px;">
                           <div style="display: inline-block;margin-left: 15px;">
                              <p style="font-size: 17px;"><?php echo $productdetails->product_name ?></p>
                              <p class="ycart_p">Select Size: {{ $productvarient->quantity }} {{ $productvarient->unit }}</p>
                           </div>
...
<script>
      ...
      if(data > 0)
      {
         var minusdata = parseInt(data) - 1;
          $.ajaxSetup({
            ...
            $.ajax({
               url: "{{ route('restaurantupdateqtycart') }}",
               type:"POST",
   ...
   function restaurantemovecartitem(cartid)
    {
         ...
         $.ajaxSetup({
            ...
            $.ajax({
               url: "{{ route('restaurantremovecartitem') }}",
               type:"POST",
               data:{cartid: cartid},
               success: function(result){
                    if(result == 1)
